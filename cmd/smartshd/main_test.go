...
func TestExecuteRequest_FailureUsesOllamaSummaryWhenAvailable(t *testing.T) {
	tempDir := t.TempDir()
	store, err := newJobStore(filepath.Join(tempDir, "jobs.db"))
	if err != nil {
		t.Fatalf("open store failed: %v", err)
	...
	mockOllama := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		_ = json.NewEncoder(writer).Encode(map[string]any{
			"response": `{"summary":"AI summarized failure","error_type":"runtime","primary_error":"mock failure","next_action":"fix command","failing_tests":[],"failed_files":[],"top_issues":["mock issue"]}`,
		})
	}))
	defer mockOllama.Close()

	t.Setenv("SMARTSH_OLLAMA_URL", mockOllama.URL)
	t.Setenv("SMARTSH_SUMMARY_ENABLED", "true")
	t.Setenv("SMARTSH_OPEN_EXTERNAL_TERMINAL", "false")
	...
	if response.SummarySource != "ollama" {
		t.Fatalf("expected ollama summary source for failed run, got %q", response.SummarySource)
	}
	if response.Summary != "AI summarized failure" {
		t.Fatalf("expected ollama summary to be used, got %q", response.Summary)
	}
}

func TestExecuteRequest_SummaryDebugFilesAreWritten(t *testing.T) {
	tempDir := t.TempDir()
	store, err := newJobStore(filepath.Join(tempDir, "jobs.db"))
	if err != nil {
		t.Fatalf("open store failed: %v", err)
	...
	mockOllama := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		_ = json.NewEncoder(writer).Encode(map[string]any{
			"response": `{"summary":"Debug summarized failure","error_type":"runtime","primary_error":"mock failure","next_action":"fix command","failing_tests":[],"failed_files":[],"top_issues":["mock issue"]}`,
		})
	}))
	defer mockOllama.Close()

	debugDir := filepath.Join(tempDir, "summary-debug")
	t.Setenv("SMARTSH_OLLAMA_URL", mockOllama.URL)
	t.Setenv("SMARTSH_SUMMARY_ENABLED", "true")
	t.Setenv("SMARTSH_SUMMARY_DEBUG_DIR", debugDir)
	t.Setenv("SMARTSH_OPEN_EXTERNAL_TERMINAL", "false")
	...
	sessions, readError := os.ReadDir(debugDir)
	if readError != nil {
		t.Fatalf("read summary debug dir failed: %v", readError)
	}
	if len(sessions) == 0 {
		t.Fatalf("expected at least one debug session directory")
	}
	sessionPath := filepath.Join(debugDir, sessions[0].Name())
	...
	for _, fileName := range requiredFiles {
		if _, statError := os.Stat(filepath.Join(sessionPath, fileName)); statError != nil {
			t.Fatalf("expected debug file %s to exist: %v", fileName, statError)
		}
	}
}
